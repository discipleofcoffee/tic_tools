...
def plot_overlay(background, overlay, xyz_direction, ncuts, threshold=1, alpha=0.5, figure_name='figure1'):

    cuts = plotting.find_cut_slices(image.load_img(background), direction=xyz_direction, n_cuts=ncuts, spacing='auto')

    print(cuts)
    n_slices = list(range(1,ncuts))
    print(n_slices)


    for ii,jj in enumerate(cuts):
        plotting.plot_stat_map(overlay, bg_img=background, cmap=bipolar(), black_bg=True,
                                        display_mode=xyz_direction, cut_coords=[jj], threshold=threshold, alpha = alpha,
