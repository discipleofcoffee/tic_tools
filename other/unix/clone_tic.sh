...
if [ ! -d $TIC_SETUP_PATH ]; then
    ...
    cp $TIC_REPOSITORY_PATH/tic_tools/other/unix/{tic,tic_tools,tic_aliases,tic_wake_software_environment}.sh $TIC_SETUP_PATH
    cp $TIC_REPOSITORY_PATH/tic_freesurfer/other/tic_freesurfer.sh $TIC_SETUP_PATH
    cp $TIC_REPOSITORY_PATH/tic_labels/other/tic_labels.sh $TIC_SETUP_PATH
